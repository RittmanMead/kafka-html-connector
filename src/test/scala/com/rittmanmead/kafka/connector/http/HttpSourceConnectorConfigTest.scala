...
class HttpSourceConnectorConfigTest extends WordSpec with Matchers {
  ...
  "JDBC Source Connector Config" should {
    ...
    "throw exception if any mandatory configuration is missing" in {
      ...
      the [IllegalArgumentException] thrownBy new HttpSourceConnectorConfig(properties).getClass
    }
    ...
    "create JDBC source configuration for incrementing mode" in {
      ...
      new HttpSourceConnectorConfig(properties).getClass shouldEqual classOf[HttpSourceConnectorConfig]
    }
    ...
    "get all properties from configuration" in {
      ...
      val configuration = new HttpSourceConnectorConfig(properties)

      configuration.getHttpUrl shouldBe "test-connection"
