...
import com.rittmanmead.kafka.connector.http.models.Mode.{IncrementingMode, TimestampMode}
          ...
          keyFieldOpt match {
            case Some(keyField) =>
              sourceRecords += new SourceRecord(
                Map(HttpSourceConnectorConstants.STORED_PROCEDURE_NAME_KEY -> storedProcedureName).asJava,
